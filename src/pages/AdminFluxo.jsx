...
export default function AdminFluxo() {
  ...
  const [vendedorId, setVendedorId] = useState(1)   // 1 = Rodrigo (padrão)
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase.from('carteira_vendedores').select('id, nome').order('id')
      setVendedores(data || [])
      setLoading(false)
    }
    load()
  ...
  useEffect(() => {
    async function loadMes() {
      const { data } = await supabase
        .from('fluxo_lojas').select('carteira_cliente_id, carros').eq('mes', mes)
      const map = {}
      ;(data || []).forEach(r => { map[r.carteira_cliente_id] = String(r.carros) })
      setFluxo(map); setOriginal(map); setMsg(null)
    }
    loadMes()
  }, [mes])
  ...
  const idsVisiveis = useMemo(() => new Set(lojas.map(l => l.id)), [lojas])
  const alterados = useMemo(() =>
    Object.keys(fluxo).filter(id =>
      idsVisiveis.has(Number(id)) && (fluxo[id]||'') !== (original[id]||'') && fluxo[id] !== ''
    ), [fluxo, original, idsVisiveis])

  const preenchidas = lojas.filter(l => fluxo[l.id] && Number(fluxo[l.id]) > 0).length
  const totalCarros = lojas.reduce((s,l) => s + (Number(fluxo[l.id]) || 0), 0)

  async function salvar() {
    if (!alterados.length) return
    setSalvando(true); setMsg(null)
    const rows = alterados.map(id => ({
      carteira_cliente_id: Number(id), mes, carros: Number(fluxo[id]) || 0,
    }))
    const { error } = await supabase.from('fluxo_lojas')
      .upsert(rows, { onConflict: 'carteira_cliente_id,mes' })
    if (error) setMsg({ tipo:'erro', txt:'Erro ao salvar: ' + error.message })
    else { setOriginal({ ...fluxo }); setMsg({ tipo:'ok', txt:`${rows.length} loja(s) salva(s) em ${mesLabel(mes)}.` }) }
    setSalvando(false)
  }

  if (loading) return <div style={{ minHeight:'100vh', background:T.bg, color:T.sub, display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'system-ui' }}>Carregando…</div>
  ...
  return (
    <div style={{ minHeight:'100vh', background:T.bg, color:T.txt, fontFamily:'system-ui, sans-serif', padding:'24px clamp(12px,3vw,40px) 120px' }}>
      <header style={{ marginBottom:20 }}>
        ...
        <div style={{ fontSize:11, letterSpacing:'0.14em', color:T.amber, fontWeight:700, textTransform:'uppercase', marginTop:12 }}>LimpAr · Coleta mensal</div>
        <h1 style={{ fontSize:'clamp(22px,4vw,30px)', fontWeight:900, margin:'6px 0 0' }}>Fluxo de Carros por Loja</h1>
      ...
      <div style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'center', marginBottom:16 }}>
        ...
        <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar loja ou bairro…"
          style={{ flex:'1 1 200px', padding:'10px 14px', borderRadius:10, border:`1px solid ${T.line}`, background:T.panel, color:T.txt, fontSize:14 }} />
      </div>

      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px,1fr))', gap:10, marginBottom:16 }}>
        {[['Lojas preenchidas', `${preenchidas} / ${lojas.length}`],
          ['Total de carros', totalCarros.toLocaleString('pt-BR')],
          ['Não salvas', alterados.length]].map(([l,v]) => (
      ...
      <div style={{ display:'grid', gap:8 }}>
        {lojasFiltradas.map(l => {
          ...
          return (
            <div key={l.id} style={{ display:'grid', gridTemplateColumns:'1fr auto', alignItems:'center', gap:12,
              padding:'10px 14px', borderRadius:10, background:T.panel2, border:`1px solid ${mudou ? T.amber+'66' : T.line}` }}>
              <div style={{ minWidth:0 }}>
                <div style={{ fontSize:13.5, fontWeight:600, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{l.nome}</div>
                <div style={{ fontSize:11, color:T.sub }}>{l.bairro || '—'}{l.situacao && l.situacao!=='Ativo' ? ` · ${l.situacao}` : ''}</div>
              </div>
        ...
        {!lojasFiltradas.length && <div style={{ color:T.sub, textAlign:'center', padding:30 }}>Nenhuma loja encontrada.</div>}
      </div>
